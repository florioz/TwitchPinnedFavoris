...
test('moderation history ignores a deletion marker carried only by a quoted reply', () => {
  const body = new HTMLElementMock();
  body.textContent = 'Ma reponse reste valide';
  body.matches = () => false;
  body.querySelector = () => null;
  const marker = new HTMLElementMock();
  const reply = new HTMLElementMock();
  reply.contains = (node) => node === marker;
  const root = new HTMLElementMock();
  root.dataset = {};
  root.textContent = 'Repond a user message supprime par un moderateur Ma reponse reste valide';
  root.classList = { contains: () => false };
  root.getAttribute = () => '';
  root.querySelectorAll = (selector) => selector.includes('.reply-line') ? [reply] : [marker];
  const tracker = new ModerationActionTracker({
    getMessageRoot: () => root,
    getMessageContainer: () => body
  });

  assert.equal(tracker.isQuotedDeletionOnly(root), true);
});

test('moderation history recognizes Twitch reply aria labels without relying on custom reply markup', () => {
  const body = new HTMLElementMock();
  body.textContent = 'La reponse est toujours visible';
  body.matches = () => false;
  body.querySelector = () => null;
  const root = new HTMLElementMock();
  root.dataset = {};
  root.textContent = 'Reponse a user message supprime par un moderateur La reponse est toujours visible';
  root.classList = { contains: () => false };
  root.getAttribute = (name) => name === 'aria-label'
    ? 'Réponse à user, envoyé à 22:29 et author : La réponse est toujours visible'
    : '';
...
test('moderation history keeps a deletion affecting the reply body itself', () => {
  const marker = new HTMLElementMock();
  const body = new HTMLElementMock();
  body.textContent = 'Message supprime par un moderateur';
  body.matches = () => false;
  body.querySelector = () => marker;
  const root = new HTMLElementMock();
  root.dataset = {};
  root.classList = { contains: () => false };
  const tracker = new ModerationActionTracker({
  ...
  assert.equal(tracker.isQuotedDeletionOnly(root), false);
});

test('moderation history never filters a root-level Twitch deletion state', () => {
  const body = new HTMLElementMock();
  const root = new HTMLElementMock();
  root.dataset = { deleted: 'true' };
  root.classList = { contains: () => false };
  const tracker = new ModerationActionTracker({
    getMessageRoot: () => root,
    getMessageContainer: () => body
