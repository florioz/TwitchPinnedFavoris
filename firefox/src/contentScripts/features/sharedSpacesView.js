(() => {
  const createAvatar = ({ url = '', label = '', className = '' }) => {
    ...
    } else avatar.textContent = String(label || '?').trim().slice(0, 1).toUpperCase();
    return avatar;
  };

  const renderMemberGallery = ({ space, permissions, t, onRoleChange }) => {
    const gallery = document.createElement('div'); gallery.className = 'tfr-shared-members__gallery';
    (space?.members || []).forEach((member) => {
      const card = document.createElement('div'); card.className = `tfr-shared-members__row is-${member.role}`; card.tabIndex = 0;
      ...
      ['editor', 'viewer'].forEach((roleId) => {
        const option = document.createElement('option'); option.value = roleId; option.textContent = t(`sharedSpaces.role.${roleId}`); roleSelect.appendChild(option);
      });
      if (member.role === 'owner') {
        const option = document.createElement('option'); option.value = 'owner'; option.textContent = t('sharedSpaces.role.owner'); roleSelect.prepend(option);
      }
      ...
      tooltip.append(tooltipName, tooltipRole, tooltipAccess); card.append(avatar, identity, roleSelect, tooltip); gallery.appendChild(card);
    });
    return gallery;
  };
  ...
  const renderInvitationInbox = ({ invitations, t, onRespond, onRefresh = () => {} }) => {
    ...
    const actions = document.createElement('div'); actions.className = 'tfr-shared-invitations__header-actions';
    const refresh = document.createElement('button'); refresh.type = 'button'; refresh.className = 'tfr-button tfr-button--ghost'; refresh.textContent = t('sharedSpaces.remote.refreshInvitations');
    refresh.addEventListener('click', async () => { refresh.disabled = true; try { await onRefresh(); } finally { refresh.disabled = false; } });
    actions.append(count, refresh); header.append(title, actions); list.appendChild(header);
    if (!invitations.length) {
      ...
      const copy = document.createElement('div'); const heading = document.createElement('strong'); heading.textContent = t('sharedSpaces.remote.inboxEmpty');
    ...
    invitations.forEach((invitation) => {
      ...
      const avatar = createAvatar({ url: invitation.invitedByAvatarUrl, label: invitation.invitedBy || invitation.spaceName, className: 'tfr-shared-invitations__avatar' });
      const content = document.createElement('div'); content.className = 'tfr-shared-invitations__content';
      const sender = document.createElement('strong'); sender.textContent = t('sharedSpaces.remote.invitationFrom', { name: invitation.invitedBy || 'Twitch' });
      const text = document.createElement('span'); text.textContent = t('sharedSpaces.remote.invitationText', { space: invitation.spaceName });
      const role = document.createElement('small'); role.textContent = t('sharedSpaces.remote.invitationRole', { role: t(`sharedSpaces.role.${invitation.role}`) });
      content.append(sender, text, role);
      const controls = document.createElement('div'); controls.className = 'tfr-shared-invitations__actions';
      ['accept', 'decline'].forEach((action) => {
        ...
        button.addEventListener('click', () => { controls.querySelectorAll('button').forEach((control) => { control.disabled = true; }); onRespond(invitation, action === 'accept'); });
        controls.appendChild(button);
      });
      row.append(avatar, content, controls); list.appendChild(row);
    });
    return list;
  };
